...
function OtpVerification() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (code.length !== 6) { // Validación básica para asegurarse de que el código OTP tenga exactamente 6 caracteres
      toast.error("Ingresa el código OTP de 6 caracteres");
      return;
    }
    // Validación adicional para permitir caracteres hexadecimales (0-9, A-F, a-f)
    if (!/^[0-9A-Fa-f]{6}$/.test(code)) {
      toast.error("El código OTP solo puede contener caracteres hexadecimales (0-9, A-F)");
      return;
    }
